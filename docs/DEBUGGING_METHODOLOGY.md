...
- On macOS Intel, heap-allocated `RTCRayHit` must be 32-byte aligned. Use `EmbreeMemory.AllocRayHit` and optionally `EmbreeMemory.ValidateRayHitAlignment` for checks.
